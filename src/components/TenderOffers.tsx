        ...
        <div className="column-content">
          <TabsContent value="pending" className="m-0 h-full">
            <div className="space-y-4">
              {pendingOffers.map(offer => <OfferCard key={offer.id} offer={offer} type="pending" />)}
              {pendingOffers.length === 0 && <EmptyState message="Aucun appel d'offres en attente" />}
            </div>
          </TabsContent>
          
          <TabsContent value="submitted" className="m-0 h-full">
            <div className="space-y-4">
              {submittedOffers.map(offer => <OfferCard key={offer.id} offer={offer} type="submitted" />)}
              {submittedOffers.length === 0 && <EmptyState message="Aucun devis soumis" />}
            </div>
          </TabsContent>
          
          <TabsContent value="completed" className="m-0 h-full">
            <div className="space-y-4">
              {completedOffers.map(offer => <OfferCard key={offer.id} offer={offer} type="completed" />)}
              {completedOffers.length === 0 && <EmptyState message="Aucun appel d'offres terminé" />}
            </div>
          </TabsContent>
        </div>
      </Tabs>
...
function OfferCard({
  ...
  return <div className="bg-secondary/30 rounded-lg p-4 transition-all hover:bg-secondary/50">
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center">
          ...
          {type === 'submitted' && <>
              <div className="flex items-center">
                {offer.isCompliant !== undefined && <div className={`flex items-center text-xs mr-3 ${offer.isCompliant ? 'text-status-completed' : 'text-status-pending'}`}>
                    {offer.isCompliant ? <>
