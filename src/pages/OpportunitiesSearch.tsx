...
export type OpportunityStatus = 'open' | 'closed' | 'assigned';
...
export default function OpportunitiesSearch() {
  ...
  const filteredOpportunities = opportunities.filter(opportunity => {
    ...
    if (searchQuery && !opportunity.projectName.toLowerCase().includes(searchQuery.toLowerCase()) && 
        ...
        !opportunity.lots.some(lot => lot.toLowerCase().includes(searchQuery.toLowerCase()))) {
      return false;
    }
    return true;
  });
  ...
  const handleOpportunitySelect = (opportunityId: string) => {
    setSelectedTender(opportunityId);
  };
        ...
        <div className="p-6">
          ...
          <Tabs defaultValue="all" className="w-full" onValueChange={setSelectedTab}>
            <div className="flex h-[calc(100vh-230px)]">
              ...
              {viewMode === 'map' ? (
                <TenderMap 
                  tenders={filteredOpportunities} 
                  onSelectTender={handleOpportunitySelect} 
