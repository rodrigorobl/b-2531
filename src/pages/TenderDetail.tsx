...
export default function TenderDetail() {
  ...
  const assignedCategories = tender.categories.filter(cat => cat.quotes.some(quote => quote.status === 'approved')).length;
  const progressPercentage = totalCategories > 0 ? Math.round(assignedCategories / totalCategories * 100) : 0;

  // Calculate budget indicators
  const totalQuotesAmount = tender.categories.reduce((total, category) => {
    const lowestQuoteForCategory = category.quotes.length > 0 ? Math.min(...category.quotes.filter(q => q.isCompliant).map(q => q.price)) : 0;
  ...
  const budgetDifferencePercentage = tender.budget > 0 ? Math.round(budgetDifference / tender.budget * 100) : 0;
  ...
  const getSortedQuotes = (quotes: Quote[]) => {
    ...
    return [...quotes].sort((a, b) => {
      ...
      if (sortConfig.key === 'date') {
        return sortConfig.direction === 'asc' ? new Date(a.submissionDate).getTime() - new Date(b.submissionDate).getTime() : new Date(b.submissionDate).getTime() - new Date(a.submissionDate).getTime();
      }
      return 0;
    });
  };
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          <div>
            ...
            <div className="flex items-center gap-3 mt-2">
              <Badge className={tender.status === 'open' ? 'bg-amber-500' : tender.status === 'assigned' ? 'bg-green-600' : 'bg-gray-500'}>
                {tender.status === 'open' ? 'En cours' : tender.status === 'assigned' ? 'Attribué' : 'Clôturé'}
              </Badge>
        ...
        <Tabs defaultValue="overview" className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="md:col-span-3">
                {selectedCategoryData ? <Card>
                    ...
                    <CardContent>
                      {selectedCategoryData.quotes.length > 0 ? <div>
                          ...
                          <div className="space-y-4">
                            {getSortedQuotes(selectedCategoryData.quotes).map(quote => <Card key={quote.id} className="overflow-hidden">
                                ...
                                <CardContent className="p-4">
                                  <div className="flex justify-between items-start">
                                    ...
                                    <div className="text-right">
                                      ...
                                      <Badge variant="outline" className={`mt-1 ${quote.isCompliant ? 'text-green-600' : 'text-red-500'}`}>
                                        {quote.isCompliant ? <><CheckCircle size={12} className="mr-1" /> Conforme</> : <><AlertTriangle size={12} className="mr-1" /> Non conforme</>}
                                      </Badge>
                                    </div>
                                  </div>
                                  
                                  {quote.comments && <div className="mt-3 text-sm bg-muted p-3 rounded-md">
                                  ...
                                  <div className="mt-4 flex justify-end gap-2">
                                    ...
                                    {quote.status === 'pending' && <Button size="sm" className="bg-green-600 hover:bg-green-700">
                  ...
                  </Card> : <div className="flex items-center justify-center h-full">
          ...
          <TabsContent value="messages">
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {tender.messages.map(message => <div key={message.id} className={`p-4 rounded-lg border ${message.senderType === 'promoteur' ? 'bg-primary/10 ml-12' : 'bg-muted'}`}>
            ...
            <Card>
              ...
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {tender.documents.map(doc => <TableRow key={doc.id}>
            ...
            <Card className="mt-6">
              ...
              <CardContent>
                <Table>
                  ...
                  <TableBody>
                    {tender.categories.filter(cat => cat.quotes.length > 0).map(category => {
                    ...
                    return <TableRow key={category.id}>
                            ...
                            <TableCell>
                              {priceDifferencePercentage ? <span className={priceDifferencePercentage > 20 ? 'text-red-500' : 'text-muted-foreground'}>
                                    {priceDifferencePercentage}%
                                  </span> : '-'}
                            </TableCell>
                            <TableCell>
                              {category.quotes.some(q => q.status === 'approved') ? <Badge className="bg-green-600">Attribué</Badge> : <Badge className="bg-amber-500">En attente</Badge>}
                            </TableCell>
                          </TableRow>;
                  })}
                  </TableBody>
